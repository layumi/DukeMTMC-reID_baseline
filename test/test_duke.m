...
p = dir('/data/uts521/zzd/DukeMTMC/bounding_box_test/*jpg');
ff = [];
%%------------------------------
for i = 1:200:numel(p)
    disp(i);
    ...
    for j=1:min(200,numel(p)-i+1)
        str = strcat('/data/uts521/zzd/DukeMTMC/bounding_box_test/',p(i+j-1).name);
...
file_name = [{p.name}];
save('resnet_gallery.mat','ff','file_name','-v7.3');

p = dir('/data/uts521/zzd/DukeMTMC/query/*jpg');
ff = [];
for i = 1:200:numel(p)
    disp(i);
    oim = [];
    for j=1:min(200,numel(p)-i+1)
        str = strcat('/data/uts521/zzd/DukeMTMC/query/',p(i+j-1).name);
        imt = imresize(imread(str),[256,256]);
        oim = cat(4,oim,imt);
    end
    f = getFeature2(net,oim,im_mean,'data','pool5n');
...
file_name = [{p.name}];
save('resnet_query.mat','ff','file_name','-v7.3');
